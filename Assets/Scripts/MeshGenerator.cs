...
public static class MeshGenerator {
    ...
    public static MeshData GenerateTerrrainMesh (float[, ] heightMap, MeshSettings meshSettings, int levelOfDetail) {
        ...
        for (int y = 0; y < numVertsPerLine; y++) {
            for (int x = 0; x < numVertsPerLine; x++) {
                ...
                bool isSkipVertex = x > 2 && x < numVertsPerLine - 3 && y > 2 && y < numVertsPerLine - 3 && ((x - 2) % skipIncrement != 0 || (y - 2) % skipIncrement != 0);
                if (isOutOfMeshVertex) {
                    vertexIndicesMap[x, y] = outOfMeshVertexIndex;
                    outOfMeshVertexIndex--;
                } else if (!isSkipVertex) {
